...
int readFromUgtxt() //从txt文件读取本科生数据
{
    ...
    do
    {
        if ((fscanf(ugFile, "%d%s", &uTmp.num, uTmp.name)) == EOF)
            break;
        fscanf(ugFile, "%d", &sex);
        uTmp.sex = (gand)sex;
...
int readFromUgdat() //从dat读取本科生数据
{
    ...
    do
    {
        if (!(fread(&uTmp, sizeof(ugdata), 1, ugFile)))
            break;
        if (count == -1)
        {
            ugHead->data.num = uTmp.num;
...
int readFromPgtxt() //从txt文件读取研究生数据
{
    ...
    do
    {
        if ((fscanf(pgFile, "%d%s", &pTmp.num, pTmp.name)) == EOF)
            break;
        fscanf(pgFile, "%d", &sex);
        pTmp.sex = (gand)sex;
...
int readFromPgdat() //从dat文件读取研究生数据
{
    ...
    do
    {
        if (!(fread(&pTmp, sizeof(pgdata), 1, pgFile)))
            break;
        if (count == -1)
        {
            pgHead->data.num = pTmp.num;
